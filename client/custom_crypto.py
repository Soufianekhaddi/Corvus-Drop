...
def chacha20_decrypt(ciphertext: bytes, key: bytes, nonce: bytes, initial_counter: int = 1) -> bytes:
    # ChaCha20 étant un chiffrement par flux, le déchiffrement est identique au chiffrement
...
def serialize_custom_private_key(d: int, password: str = None) -> str:
    """
    Chiffre (si mot de passe) et sérialise la clé privée ECC en format PEM personnalisé.
    """
    key_data = {}
    if password:
        salt = os.urandom(16)
        # Dérivation d'une clé de chiffrement ChaCha20 de 256 bits à partir du mot de passe
        derived_key = pbkdf2_hmac_sha256(password.encode("utf-8"), salt, 2000, 32)
        nonce = os.urandom(12)
        
        # Le contenu en clair de la clé privée est son scalaire d en chaîne hexadécimale
...
def deserialize_custom_private_key(pem_str: str, password: str = None) -> int:
    """
    Déchiffre et charge la clé privée à partir du format PEM personnalisé.
    """
    try:
...
def serialize_custom_public_key(point: ECCPoint) -> str:
    """
    Sérialise la clé publique en format PEM personnalisé.
    """
    key_data = {
        "x": hex(point.x),
        "y": hex(point.y)
...
def deserialize_custom_public_key(pem_str: str) -> ECCPoint:
    """
    Charge la clé publique depuis le format PEM personnalisé.
    """
    try:
        lines = pem_str.strip().split("\n")
        body_lines = [l for l in lines if not l.startswith("-----")]
...
def create_custom_certificate(subject_cn: str, issuer_cn: str, validity_days: int, public_key: ECCPoint, issuer_private_key: int) -> str:
    """
    Génère un Certificat Electronique Customisé signé avec notre ECDSA from scratch.
    """
    cert_data = {
        "subject": {
            "CN": subject_cn
...
def verify_custom_certificate(cert_pem: str, issuer_public_key: ECCPoint) -> bool:
    """
    Vérifie l'authenticité d'un certificat à l'aide de la clé publique de l'émetteur.
    """
    try:
